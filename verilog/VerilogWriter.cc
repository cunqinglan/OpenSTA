  ...
  while (pin_iter->hasNext()) {
    ...
    if (term) {
      ...
      if (!net) {
        ...
        const std::string dbg_port_name = dbg_port ? network_->name(dbg_port) : "?";
        const std::string dbg_cell_name = network_->cellName(inst);
        fprintf(stderr, "WARNING: writeAssigns: term has no net "
                "for port '%s' of module '%s'\n",
                dbg_port_name.c_str(),
                dbg_cell_name.empty() ? "?" : dbg_cell_name.c_str());
        continue;
      }
